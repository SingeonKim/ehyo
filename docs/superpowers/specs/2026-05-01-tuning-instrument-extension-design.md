...
기존 그대로: Next.js 15, Zustand persist v10 → v11, Tailwind v4, Vitest + Testing Library + Playwright.

---

## Scope

**작업 단위**: 7 tuning preset + 4 voice mute toggle + UI 컨트롤 4개 + persist v11 마이그레이션 + 테스트 확장
...
case 10: {
  const next = { ...state };
  next.fretboard = { ...next.fretboard, tuning: 'guitar-6-standard' };
  next.backing = {
    ...next.backing,
...
- `tests/unit/lib/store/migrations.test.ts` (확장)
  - v10 → v11: `fretboard.tuning = 'guitar-6-standard'`, `backing.voiceMutes` 4개 false
  - 다른 필드 무결성
...
- v10 → v11 마이그레이션 후 기존 사용자 카드 재생/지판 정상
...
| `nextjs-architect` | persist v11 + store 셀렉터 |
...
| persist v10 → v11 마이그레이션 실패 | 기존 사용자 설정 리셋 | `__migrate` export로 unit test, default 폴백 |
...
- [ ] `nextjs-architect` v11 마이그레이션 검증
