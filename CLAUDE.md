...
# README/마케팅 스크린샷 14장 자동 캡처 (Docker Playwright)
pnpm screenshots             # docs/introduction/auto/ 에 결정론적으로 14장 생성
                             # 검수 후 본 폴더(docs/introduction/)로 직접 이전.
                             # auto/는 .gitignore.
```
...
- **카드 프로필 시스템 (Sprint 9 PR-B/D + Sprint 10·11)**: 카탈로그 29장 모두 `lib/audio/backing/card-profiles.ts`의 `CARD_PROFILES`에 등재. 카드별 부분 override: `rhythmVariant`(카테고리 selectSlot 분기 키) + `toneProfile`(velocityScale / voiceGain × 4 voice / reverbWet) + `instrumentOverrides`(부분 instrument 교체). `profile-merge.ts`의 `resolveCardProfile(slug, category)`이 카테고리 default 위에 카드 override 머지(voiceGain만 한 단계 깊은 머지, 그 외 얕은 머지). 미등재 슬러그 → 빈 프로필 fallback. dev에서 `__assertCardProfilesMatch(catalogSlugs)`가 백엔드 카탈로그와 정합성 검증. instrument override는 SoundFont 캐시 부담 고려해 5장만 사용(slow-minor-blues clean / hard-bop-minor-blues + jazz-major-blues jazz archtop / phrygian-vamp distortion / power-ballad-rock electric_guitar_clean).
...
- `apps/web/scripts/capture-screenshots.mjs` — README/마케팅용 14장 자동 캡처 스크립트(Playwright). `pnpm screenshots`(루트)가 `docker-compose.test.yml`의 `screenshots` profile로 실행. 셀렉터 안정 hook: `[data-category]`(카탈로그 카테고리 wrapper), `[data-testid="highlight-colors"]`(ImportantDegreesToggle 패널), `[data-testid="progression-card-<slug>"]`(카드 래퍼). 마디 진행 시점은 `bar X/12` 텍스트 polling으로 잡는다(BPM 무관 결정론).
