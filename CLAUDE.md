...
- `app/` — Next.js App Router. `(practice)` 라우트 그룹이 메트로놈·지판·jam 뷰를 공유 레이아웃으로 묶음. 헤더의 `MetronomeDock`은 layout 상주(다른 페이지에서도 메트로놈 유지). 페이지는 정적 쉘만, 실제 UI는 components/ 클라이언트 컴포넌트. 사용자 노출 라벨은 *Practice*(jam 페이지의 표시명, 탑바 네비, 홈 CTA) — 라우트 디렉토리/URL은 `/jam` 유지(북마크 호환).
