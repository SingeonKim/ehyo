...
sticky 지판 + **29장** 코드 진행 카탈로그(blues / pop / jazz / minor / modal / funk / bossa / folk / rock 9 카테고리).

Roman ↔ Absolute 표기 토글, 4-voice mute(drums/bass/guitar/aux), 키·BPM·볼륨 컨트롤.

아래는 Key=A · Label=Degree로 **12-Bar Blues (Quick Change)** 의 10번째 마디(IV7) 재생 중 — 지판은 코드 톤이 IV로 따라가고, 카드의 chord chip은 현재 마디를 강조합니다.
...
배킹 트랙은 [smplr](https://github.com/danigb/smplr) 기반 SoundFont · DrumMachine · Reverb 합성 — 브라우저에서 직접 생성합니다. 카드별로 instrument · swing · 마디 변주를 차등화해 같은 12-bar라도 jazz / shuffle / jump가 다른 질감을 가집니다. 슬래시 코드(`I/VII` 등)는 베이스 음을 별도로 분리해 Travis fingerstyle의 descending bass 같은 idiom을 그대로 재현합니다.

### 라이트 테마

헤더 우측 sun/moon 아이콘 버튼으로 라이트/다크 1클릭 전환. 라이트 팔레트는 *악보지/편집지* 톤 — cream 배경 + 짙은 갈색 잉크 + 어두운 황동 accent. 모든 토큰이 cream 배경 대비 AA 4.5:1 보장. 사용자 선택은 `localStorage`에 영속화되며, 새로고침 시 첫 페인트부터 라이트(FOUC 없음). 첫 방문은 항상 다크가 기본입니다.
...
apps/
  web/                    Next.js 프론트엔드 (@my-music-app/web)
    ...
    components/
      ...
      ui/                 도메인 비특정 공통 UI (ThemeSync, ThemeToggle)
    lib/
...
- **테마 시스템 = 토큰 재정의**: 다크가 `@theme` 기본값. 라이트는 `html[data-theme="light"]` selector가 동일 토큰을 cream + dark brass로 덮어씀 — 컴포넌트 코드 0줄 수정. FOUC 방지는 `next/script beforeInteractive`가 hydration 이전에 처리.
...
- [x] **Phase 4** — `/jam` (Practice) 통합 뷰
  ...
  - **카탈로그 29장** (Sprint 11까지) — folk/rock 카테고리 신규 진입, modal 4bar 통일, Autumn Leaves form / Cissy Strut form / Travis fingerstyle / Power Ballad / Punk garage 등 idiom 단위 카드
  - 슬래시 코드 파서(`chordBassMidi`로 descending bass) + guitar `voicingMode='power'` + drums tom/crash kit별 sample 동적 lookup
  - 절대 볼륨 통일(velocityScale/voiceGain override 폐지) — 카드 정체성은 reverbWet · instrument override · 패턴 변주로만 표현
  - **v1.1.0 (2026-05-05)** — 라이트 테마 토글. `html[data-theme="light"]` 토큰 재정의(악보지 cream + dark brass), FOUC 가드, AA 명도 보정. Store 마이그레이션 없음(v12 `ui.theme` dead state 활성화).
